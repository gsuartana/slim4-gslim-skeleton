...
class MakeTrait extends Command
{
    ...
    {
        $this->setDescription('Create interface trait')
            ->addArgument(
                'classname',
    ...
    {
        $className = MakeHelper::setClassName($input->getArgument('classname'), "Trait");
        try {
            list(
                $className,
                $path,
                $fileName
                        ...
                        $className,
                        'src/App' . DIRECTORY_SEPARATOR . 'Traits'
                    );
        } catch (Exception $e) {
        ...
        // Controller
        $text = file_get_contents(__DIR__.'/Templates/trait.template.tpl');
